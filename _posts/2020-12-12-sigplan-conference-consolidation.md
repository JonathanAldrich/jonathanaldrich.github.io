...
For papers of interest across multiple SIGPLAN conferences, it's nice to have 4 deadlines around the year for major SIGPLAN conferences, because work matures at different times, and because timely resubmission becomes possible after fixing issues with a paper identified by reviewers.  But if ratings-driven consolidation continues, though, this benefit is threatened.  Furthermore, many papers are not good matches for every SIGPLAN conference, so even today this benefit only applies to a limited subset of papers.  It would be nice to do better.
...
There are also known models for fixing the submission calendar issue: journal-first submission (e.g. as in TOPLAS submission, with later presentation in any of 3 SIGPLAN conferences) and multiple deadlines (e.g. as the Programming conference/journal hybrid does).  For SIGPLAN, the TOPLAS option is already in place, and I would personally be pretty happy with that.  If there are those who aren't, a Programming-style solution, where committee members agree to be on a conference committee for a year, with 3 different submission windows, might be a good alternative.

In addition to hybrid in-person/remote conference organizations, an interesting alternative is "satellite" conferences, such as one by Steve Blackburn recently during SPLASH'20.  These more local gatherings, held synchronously with an international conference, can increase participation among those who can't travel to the international event.


Concrete Proposal: A SIGPLAN Conference
---------------------------------------

I propose that we create an International Conference on Programming Languages (ICPL), meeting in-person annually, with remote attendance and satellite events supported to the extent possible--so those with fewer financial resources or who wish to avoid travel for climate reasons can still participate.  The location would rotate among continents, with a bias towards locations with a lower overall carbon footprint, but still including (for example) Asia in the rotation.  ICPL would replace ICFP, OOPSLA, PLDI, and POPL, both as an event and as a publication venue, and it would host a wide variety of specialized workshops and colocated conferences.  If we act quickly, the first one could be held in June/July 2022 (benefiting from summer academic break in the northern hemisphere, while avoiding August holidays) by which time we can hope COVID would be well under control.  All other SIGPLAN-sponsored conferences would be colocated with ICPL or held virtually.  ICPL papers would be published in PACMPL, as ICFP/OOPSLA/POPL papers are today.  To preserve and even enhance intellectual diversity, the PC would have subcommittees covering communities represented across SIGPLAN; we would look to CHI for organizational inspiration.  ICPL would accept submissions throughout the year, with a late fall deadline for papers expected to be presented in any given year's conference.  TOPLAS to ICPL would be one option for submission throughout the year, but I think it would also be good to have a standing committee where members agree to review a bounded number of papers over a year (and re-review any of those that are resubmitted the following year, journal-style).  The new conference would target publishing at least as many papers annually as the four major SIGPLAN conferences currently do when combined, and maintaining an acceptance rate that remains selective yet, for inclusivity, is at or above the average of those four conferences.
